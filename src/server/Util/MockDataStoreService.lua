...
if RunService:IsClient() then
	warn(
		"Mocked DataStoreService is functioning on the client: The real DataStoreService will not work on the client"
	)
end
...
function API:GetDataStore(name, scope)
	assert(
		type(name) == "string",
		"DataStore name must be a string; got" .. type(name)
	)
	assert(
		type(scope) == "string" or scope == nil,
		"DataStore scope must be a string; got" .. type(scope)
	)
	...
	function d.OnUpdate(_s, k, onUpdateFunc)
		assert(
			type(onUpdateFunc) == "function",
			"Update function argument must be a function"
		)
...
function API:GetOrderedDataStore(name, scope)
	...
	function d.GetSortedAsync(_s, isAscending, pageSize, minValue, maxValue)
		assert(
			type(pageSize) == "number" and math.floor(pageSize) > 0,
			"PageSize must be an integer and greater than 0"
		)
		assert(
			minValue == nil or type(minValue) == "number",
		...
		if minValue and maxValue then
			assert(
				minValue <= maxValue,
				"MinValue must be less or equal to MaxValue"
			)
		end
		...
		for k, v in pairs(allData) do
			local pass = (
					(not minValue or v >= minValue)
					and (not maxValue or v <= maxValue)
				)
			if pass then
